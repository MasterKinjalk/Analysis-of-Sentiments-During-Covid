...
def preprocess_cum_global_data(df):
    df['tweet_date'] = pd.to_datetime(df['tweet_date'])
    df.sort_values('tweet_date', inplace=True)
    grouped_df = df.drop(columns='country/region').groupby('tweet_date', as_index=False).mean().sort_values('tweet_date')
...
def plot_cumulative_global_emotions():
    ...
    for col in df_cumulative.columns[1:]:  # Skip the timestamp column
        df_cumulative[col] = df_cumulative[col].rolling(window=window_size, min_periods=1).mean()
...
def plot_cumulative_country_emotions(country):
    ...
    selected_country_df = country_df_cum[country_df_cum['country/region'] == country].drop(columns='country/region')
