...
df = pd.read_csv('processedData/sampled_cleaned_covid_twitter_data.csv', usecols=['tweet_timestamp', 'country/region', 'fear_intensity', 'anger_intensity', 'happiness_intensity', 'sadness_intensity'])

# Convert timestamps to datetime, extract date, and sort
df['tweet_timestamp'] = pd.to_datetime(df['tweet_timestamp'], format='%Y-%m-%d %H:%M:%S')
df['tweet_date'] = df['tweet_timestamp'].dt.date
