...
def preprocess_global_data(df):
    ...
    for column in columns_to_enhance:
        data = post_cutoff_df[
            column
        ].dropna()  # Drop NA values to ensure detection works
        if not data.empty:
...
def preprocess_country_data(df):
    ...
    df = (
        df.groupby(["tweet_date", "country/region"])
        .mean()
        .reset_index()
...
important_dates_dict = {
    "2020-02-11": "WHO names Covid-19",
    "2020-03-11": "WHO declares pandemic",
    "2020-04-02": "Worldwide cases at 1 million",
    "2020-04-13": "US ceases WHO funding",
    "2020-04-17": "First Moderna trials",
    "2020-04-24": "ACT-A announced",
    "2020-08-30": "Schools reopen",
    "2020-10-02": "Trump infected",
}

dates_US = {
    "2020-03-06": "Grand Princess cruise ship",
    "2020-03-15": "States begin lockdown",
    "2020-04-03": "CDC mask guidelines",
    "2020-04-30": "Warp Speed",
    "2020-10-02": "Trump infected",
}
dates_UK = {
    "2020-03-23": "Lockdown starts",
...
def plot_global_emotions(selected_date):
    ...
    labels = {
        ...
        "fear_intensity": "Fear Intensity",
        "happiness_intensity": "Happiness Intensity",
    ...
    fig.add_scatter(
        x=pd.to_datetime([selected_date, selected_date, selected_date, selected_date]),
        y=global_df.loc[pd.to_datetime(selected_date)],
        name="Current Day",
        mode="lines+markers",
        marker=dict(size=10),
    )
...
def plot_country_emotions(selected_date, country):
    ...
    fig = px.line(
        selected_country_df,
        x=selected_country_df.index,
        y=selected_country_df.columns,
        title=title,
        labels=labels,
        render_mode="svg",
        line_shape="spline",
    ...
    fig.add_scatter(
        x=pd.to_datetime([selected_date, selected_date, selected_date, selected_date]),
        y=selected_country_df.loc[pd.to_datetime(selected_date)],
        name="Current Day",
        mode="lines+markers",
        marker=dict(size=10),
    )
